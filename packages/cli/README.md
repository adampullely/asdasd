...
`asdasd` — first-class TypeScript CLI for the My App control
...
This package is internal to the `asdasd` monorepo and is not
...
```
asdasd login    [--api-url=URL] [--token=BEARER]
asdasd logout
asdasd whoami
```
...
```
asdasd org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
asdasd project create <name> [--idempotency-key=KEY]
asdasd env create <project-id> <name> [--idempotency-key=KEY]
asdasd api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
asdasd webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
asdasd usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
asdasd billing summary
asdasd audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`asdasd org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `asdasd login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/asdasd/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/asdasd/config.json` (mode 0644, not a secret). Override
both via `ASDASD_CONFIG_DIR` (used by tests).

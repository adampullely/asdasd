...
describe("resolveConfigDir", () => {
  it("respects ASDASD_CONFIG_DIR override", () => {
    const prev = process.env["ASDASD_CONFIG_DIR"];
    process.env["ASDASD_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["ASDASD_CONFIG_DIR"];
      else process.env["ASDASD_CONFIG_DIR"] = prev;
    }
  });
});

...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new MyApp({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => MyAppError]> = [
  ...
  it("forward-compat: unknown error code → base MyAppError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(MyAppError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as MyAppError).code).toBe("future_quota_exceeded");
    }
  });

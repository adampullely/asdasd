// Base HTTP transport for the MyApp SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `MyAppError`
...
export interface ClientOptions {
  /**
   * Base URL of the MyApp api-edge worker, e.g.
   * `https://api.myapp.dev`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `MyApp` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "MyApp SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "MyApp SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}

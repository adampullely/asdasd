...
My App TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the My App control plane API.

## Install (workspace)

This package is internal to the `asdasd` monorepo. Add it as a
...
```ts
import { MyApp } from "@saas/sdk";

const client = new MyApp({
  baseUrl: "https://api.myapp.dev",
  auth: { kind: "bearer", token: process.env["ASDASD_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `MyAppError`. Branch on
...
| `MyAppError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `MyAppError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new MyApp({ fetch })`.
